"""Wabi-Sabi variant — generate the 8 project pages.

Adapted from the root _gen_stubs.py. Differences:
- Asset paths to project content use `../../` prefix (variant lives 2 levels deep).
- Brand assets remain local to the variant folder.
- Sumi/washi palette + Newsreader body + Cormorant Garamond display.
- Brush-stroke separators (SVG paths) instead of brass hairlines.
- Sepia-tinted hero photos and slight rotation on chapter heads for hand-placed feel.
- Same wildcards per slug as the root variant.
"""
...
PROJECTS_ROOT = VARIANT_ROOT.parent.parent  # `Hadeel Algheshaian/`
IMG_EXT = {".jpg", ".jpeg", ".png", ".webp"}
PREFIX = "../../"  # to reach project asset folders from variant pages
...
BIO_BY_SLUG = {
    "almamlaka":  {"en": "ALMAMLAKA gathers Najdi proportion and modern restraint into a quiet civic interior. The brief was to make a building feel inevitable from the moment you cross its threshold.", "ar": "يجمع مشروع المملكة بين النسب النجدية والانضباط المعاصر في فضاء مدني هادئ. كان المطلوب أن يبدو المبنى وكأنه كان دائماً هنا منذ اللحظة التي تعبر فيها عتبته."},
    ...
    for sub in sorted(folder.iterdir(), key=lambda p: parse_order(p.name)[0]):
        ...
        for f in sorted(sub.rglob("*")):
            if f.is_file() and f.suffix.lower() in IMG_EXT:
                rel = f.relative_to(PROJECTS_ROOT).as_posix()
                images.append(PREFIX + rel)
        if images:
...
WILDCARD_CSS = {
    "rotated_edge": """
.gallery-item.rotated-edge { transform: rotate(-3deg); margin: 0 -10px; z-index: 2; box-shadow: 0 12px 32px rgba(0,0,0,0.45); }
""",
    "texture_paper": """
...
.chapter-block::before {
  ...
  background:
    radial-gradient(ellipse at 30% 40%, rgba(85, 60, 35, 0.18) 0%, transparent 55%),
    radial-gradient(ellipse at 75% 65%, rgba(85, 60, 35, 0.14) 0%, transparent 55%);
...
.chapter-block::after {
  ...
  background-image:
    repeating-linear-gradient(0deg,   rgba(236,228,208,0.18) 0 1px, transparent 1px 4px),
    repeating-linear-gradient(90deg,  rgba(236,228,208,0.18) 0 1px, transparent 1px 4px);
}
:root[data-theme="light"] .chapter-block::after { opacity: 0.24; background-image:
    repeating-linear-gradient(0deg,   rgba(42,33,26,0.10) 0 1px, transparent 1px 4px),
    repeating-linear-gradient(90deg,  rgba(42,33,26,0.10) 0 1px, transparent 1px 4px); }
""",
    "program_marquee": """
.marquee {
  position: relative; overflow: hidden;
  border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);
  margin: 32px 0; height: 64px; display: flex; align-items: center;
  font-family: var(--font-serif); font-style: italic; font-size: 24px; color: var(--accent);
}
.marquee-track { display: flex; gap: 60px; padding-left: 60px; animation: marquee-scroll 44s linear infinite; white-space: nowrap; }
.marquee-track span::after { content: " · "; color: var(--muted); margin-left: 60px; }
@keyframes marquee-scroll { from { transform: translateX(0); } to { transform: translateX(-50%); } }
@media (prefers-reduced-motion: reduce) { .marquee-track { animation: none; } }
""",
    "plan_hotspots": """
.plan-wrap { position: relative; max-width: 980px; margin: 0 auto 72px; }
.plan-wrap img { width: 100%; display: block; filter: sepia(0.18) contrast(1.04) brightness(0.96); }
.plan-hotspot {
  position: absolute; width: 22px; height: 22px;
  border: 1.5px solid var(--accent); border-radius: 50%;
  background: var(--bg); cursor: pointer;
  transform: translate(-50%, -50%); transition: transform 0.4s, background 0.4s;
}
.plan-hotspot::before { content: ""; position: absolute; inset: 5px; background: var(--accent); border-radius: 50%; opacity: 0.65; }
.plan-hotspot:hover { transform: translate(-50%, -50%) scale(1.5); background: var(--accent); }
...
.meta-sticker dd { color: var(--accent); margin: 0; font-family: var(--font-serif); font-style: italic; font-size: 15px; letter-spacing: 0; text-transform: none; }
""",
    "color_chips": """
.chips { display: flex; gap: 22px; margin: 32px 0; flex-wrap: wrap; justify-content: center; }
.chip { display: flex; flex-direction: column; align-items: center; gap: 8px; transform: rotate(var(--cr, 0deg)); }
.chip:nth-child(odd) { --cr: -1deg; }
.chip:nth-child(even) { --cr: 1deg; }
.chip-swatch { width: 72px; height: 72px; border: 1px solid var(--border); }
.chip-name { font-family: var(--font-serif); font-style: italic; font-size: 15px; color: var(--text); }
...
.selfdraw.live path, .selfdraw.live line, .selfdraw.live rect, .selfdraw.live polygon { stroke-dashoffset: 0; }
""",
}
...
<title>__TITLE_EN__ — Hadeel Algheshaian (Wabi-Sabi)</title>
<link rel="icon" type="image/svg+xml" href="favicon.svg">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400;1,500&family=Newsreader:ital,opsz,wght@0,6..72,300;0,6..72,400;1,6..72,300&family=Tajawal:wght@300;400;500&display=swap" rel="stylesheet">
<script>
  (function () {
...
<style>
  :root {
    --bg: #1F1814; --surface: #2A211A; --border: #3A2F25;
    --text: #ECE4D0; --muted: #8A7E6B; --accent: #B5755A; --accent-2: #7A8A5C;
    --nav-bg: rgba(31, 24, 20, 0.86);
    --paper-tint: rgba(236, 228, 208, 0.04);
    --hero-bg-filter: sepia(0.4) saturate(0.65) brightness(0.42) contrast(1.0);
    --font-en: 'Newsreader', Georgia, serif;
    --font-ar: 'Tajawal', system-ui, sans-serif;
    --font-serif: 'Cormorant Garamond', 'Times New Roman', serif;
    --rule: rgba(181, 117, 90, 0.5);
  }
  :root[data-theme="light"] {
    --bg: #ECE4D0; --surface: #DCD3BC; --border: #BAA985;
    --text: #2A211A; --muted: #6B5C45; --accent: #8B4F32; --accent-2: #6B7F5A;
    --nav-bg: rgba(236, 228, 208, 0.94);
    --paper-tint: rgba(42, 33, 26, 0.04);
    --hero-bg-filter: sepia(0.3) saturate(0.85) brightness(0.95) contrast(0.95);
    --rule: rgba(139, 79, 50, 0.55);
  }
  *, *::before, *::after { box-sizing: border-box; }
  html, body { margin: 0; padding: 0; }
  body { background: var(--bg); color: var(--text); font-family: var(--font-en);
    font-weight: 300; font-size: 16px; line-height: 1.85; -webkit-font-smoothing: antialiased; overflow-x: hidden;
    background-image:
      radial-gradient(circle at 18% 22%, var(--paper-tint) 0%, transparent 40%),
      radial-gradient(circle at 78% 68%, var(--paper-tint) 0%, transparent 35%); }
  ...
  .nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; direction: ltr;
    ...
    padding: 18px 44px; background: var(--nav-bg);
    backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); }
  .nav::after { content: ""; position: absolute; left: 4%; right: 4%; bottom: 0; height: 1px;
    background: linear-gradient(to right, transparent 0%, var(--rule) 18%, var(--rule) 82%, transparent 100%); opacity: 0.7; }
  .nav-logo { display: flex; align-items: center; gap: 14px; cursor: pointer; }
  .nav-logo-mark { display: block; width: 38px; height: 38px; background-color: var(--text);
    -webkit-mask: url("brand/logo-mark.png") center / contain no-repeat;
            mask: url("brand/logo-mark.png") center / contain no-repeat;
    animation: logo-spin 38s linear infinite; transform-origin: 50% 50%;
    filter: drop-shadow(0 1px 2px rgba(0,0,0,0.3)); opacity: 0.92; }
  .nav-logo-name { font-family: var(--font-serif); font-style: italic; font-weight: 400; font-size: 19px; letter-spacing: 0.02em; }
  .nav-links { display: flex; align-items: center; gap: 36px; font-size: 11px; letter-spacing: 0.28em; text-transform: uppercase; color: var(--muted); }
  .nav-links a { transition: color 0.4s; }
  .nav-links a:hover { color: var(--accent); }
  .nav-toggles { display: flex; align-items: center; gap: 12px; }
  .nav-burger { display: none; background: transparent; border: 1px solid var(--border); color: var(--text); width: 36px; height: 36px; border-radius: 50%; cursor: pointer; align-items: center; justify-content: center; padding: 0; }
  .nav-burger span, .nav-burger span::before, .nav-burger span::after { display: block; width: 18px; height: 1.5px; background: currentColor; }
  .nav-burger span { position: relative; }
  .nav-burger span::before { content: ""; position: absolute; left: 0; top: -6px; }
  .nav-burger span::after { content: ""; position: absolute; left: 0; top: 6px; }
  ...
  .toggle-btn { width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center;
    border: 1px solid var(--border); background: transparent; color: var(--text); border-radius: 50%;
    font-family: var(--font-en); font-size: 10px; cursor: pointer; transition: border-color 0.3s, color 0.3s; }
  .toggle-btn:hover { border-color: var(--accent); color: var(--accent); }
  .theme-icon-light, .theme-icon-dark { width: 14px; height: 14px; stroke: currentColor; fill: none; stroke-width: 1.4; }
  ...
  .brush-rule { display: block; width: 220px; height: 22px; margin: 12px auto 0; color: var(--accent); opacity: 0.85; }
  .brush-rule path { stroke: currentColor; stroke-width: 4; stroke-linecap: round; fill: none; }

  .project-hero-wrap {
    position: relative; isolation: isolate;
    margin-top: 76px;  /* clear the fixed nav */
    min-height: 86vh;
    overflow: hidden;
  }
  ...
  .project-hero-overlay-top {
    ...
    background: linear-gradient(to bottom, rgba(15, 10, 6, 0.65) 0%, rgba(15, 10, 6, 0.32) 55%, rgba(15, 10, 6, 0) 100%);
  }
  .project-hero-overlay-bottom {
    position: absolute; bottom: 0; left: 0; right: 0; height: 50%;
    pointer-events: none; z-index: 1;
    background: linear-gradient(to top, rgba(15, 10, 6, 0.82) 0%, rgba(15, 10, 6, 0.45) 35%, rgba(15, 10, 6, 0) 100%);
  }
  .project-hero-title {
  ...
  .ph-name {
    ...
    color: #ECE4D0; margin: 0;
    text-shadow: 0 2px 14px rgba(0,0,0,0.6);
  }
  .ph-rule { display: block; width: 110px; height: 14px; margin: 16px auto 0; color: var(--accent); opacity: 0.9; }
  .ph-rule path { stroke: currentColor; stroke-width: 3; stroke-linecap: round; fill: none; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.5)); }
  .ph-year {
    margin-top: 12px;
    font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase;
    color: rgba(236, 228, 208, 0.85);
    text-shadow: 0 1px 4px rgba(0,0,0,0.55);
  }
  .project-hero-desc {
  ...
  .project-hero-desc .desc-eyebrow {
    ...
    font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase;
    color: rgba(236, 228, 208, 0.82); margin-bottom: 14px;
    text-shadow: 0 1px 3px rgba(0, 0, 0, 0.55);
  }
  .project-hero-desc .desc-body {
    font-family: var(--font-en); font-weight: 300;
    font-size: 15px; line-height: 1.85; color: #ECE4D0;
  ...
  @media (max-width: 700px) {
    ...
    .project-hero-desc .desc-body { font-size: 14px; line-height: 1.7; }
  }

  .section-header { text-align: center; padding: 110px 40px 22px; }
  .section-header.chapter-block { position: relative; }
  .section-eyebrow { font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase; color: var(--muted); position: relative; z-index: 1; }
  .section-title { font-family: var(--font-serif); font-style: italic; font-weight: 400; font-size: 32px; margin: 8px 0 0; position: relative; z-index: 1; }
  ...
  .gallery-row { display: flex; gap: 6px; margin-bottom: 6px; align-items: stretch; }
  .gallery-row > .gallery-item { flex: var(--ar, 1) 1 0; min-width: 0; cursor: pointer; overflow: hidden; position: relative; }
  .gallery-row > .gallery-item img { width: 100%; height: 100%; object-fit: cover; display: block;
    filter: sepia(0.05); transition: filter 0.6s, transform 1.4s ease-out; }
  .gallery-row > .gallery-item:hover img { filter: sepia(0); transform: scale(1.02); }
  /* center: trailing odd image, half-width, natural aspect */
  .gallery-row.center { justify-content: center; align-items: flex-start; }
  .gallery-row.center > .gallery-item { flex: 0 0 calc(50% - 3px); }
  ...
  .description-eyebrow { font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase; color: var(--muted); margin-bottom: 18px; }
  .description-title { font-family: var(--font-serif); font-style: italic; font-weight: 400; font-size: 30px; margin: 0 0 22px; }
  .description-body { font-size: 16px; line-height: 1.95; color: var(--text); }

  .lightbox { position: fixed; inset: 0; background: #110b07; display: none; z-index: 9000; flex-direction: column; cursor: zoom-out; }
  ...
  .lightbox-counter { position: absolute; top: 18px; left: 24px; color: var(--text); font-family: var(--font-en); font-size: 11px; letter-spacing: 0.32em; }
  ...
  .lightbox-prev, .lightbox-next { position: absolute; top: 50%; transform: translateY(-50%); background: transparent; border: 1px solid rgba(236,228,208,0.3); color: var(--text); width: 48px; height: 48px; border-radius: 50%; cursor: pointer; font-size: 20px; }
  .lightbox-prev { left: 24px; } .lightbox-next { right: 24px; }
  .lightbox-prev:hover, .lightbox-next:hover { border-color: var(--accent); color: var(--accent); }
  .lightbox-strip { display: flex; gap: 6px; padding: 12px; overflow-x: auto; background: #080503; border-top: 1px solid #2A211A; height: 96px; flex-shrink: 0; }
  .lightbox-strip-thumb { flex-shrink: 0; width: 96px; height: 72px; cursor: pointer; opacity: 0.45; transition: opacity 0.3s; border: 1px solid transparent; overflow: hidden; }
  ...
  .pf { padding: 56px 40px; text-align: center; border-top: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; gap: 24px; flex-wrap: wrap; max-width: 1280px; margin: 0 auto; }
  .pf a { color: var(--muted); font-size: 11px; letter-spacing: 0.28em; text-transform: uppercase; }
  .pf a:hover { color: var(--accent); }
  .pf .pf-title { font-family: var(--font-serif); font-style: italic; font-size: 17px; color: var(--text); }

  @media (max-width: 900px) {
    .nav { padding: 14px 22px; }
    .nav-burger { display: inline-flex; }
    .nav-links { display: none; position: fixed; top: 64px; left: 0; right: 0; background: var(--nav-bg);
      backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px);
      flex-direction: column; align-items: flex-start; gap: 0; padding: 18px 28px 28px;
      border-bottom: 1px solid var(--border); font-size: 13px; letter-spacing: 0.18em; }
    .nav-links a { padding: 14px 0; width: 100%; border-bottom: 1px solid var(--border); }
    .nav-links a:last-child { border-bottom: none; }
    .nav.open .nav-links { display: flex; }
    .nav-logo-name { font-size: 17px; }
    .project-head { padding: 110px 22px 44px; min-height: 320px; }
    .gallery { padding: 0 22px; }
    .description { padding: 64px 22px 48px; }
    .pf { padding: 40px 22px; }
    .lightbox-strip { height: 76px; } .lightbox-strip-thumb { width: 76px; height: 58px; }
  }
...
<nav class="nav">
  ...
  <div class="nav-toggles">
    ...
    <button class="nav-burger" id="nav-burger" aria-label="Menu" aria-expanded="false"><span></span></button>
  </div>
</nav>
...
<div class="project-hero-wrap">
  ...
  <div class="project-hero-title">
    ...
    <svg class="ph-rule" viewBox="0 0 110 14" preserveAspectRatio="none" aria-hidden="true">
      <path d="M 4,8 Q 30,2 56,8 T 106,7" />
    </svg>
...
<script>
  ...
  function balanceGallery() { /* column-flow handles layout */ }
  ...
  function paintLightbox() {
    document.getElementById('lightbox-img').src = ALL_IMAGES[lbIndex];
  ...
  function buildStrip() {
    const strip = document.getElementById('lightbox-strip'); strip.innerHTML = '';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.nav-links a').forEach(a => a.addEventListener('click', () => navEl.classList.remove('open')));
...
def gallery_html(sections, all_images_offset=0):
    ...
    for s in sections:
        out.append('<section class="section-header chapter-block">')
        out.append(f'<h2 class="section-title" data-en="{s["label_en"]}" data-ar="{section_ar(s["label_en"])}">{s["label_en"]}</h2>')
        out.append('<svg class="brush-rule" viewBox="0 0 220 22" preserveAspectRatio="none" aria-hidden="true"><path d="M 6,12 Q 50,4 110,12 T 214,11" /></svg></section>')
...
def wildcard_for(slug, project, sections):
    ...
    if project["wildcard"] == "rotated_edge":
        ...
        extra_html_before = f'<div class="marquee" aria-hidden="true"><div class="marquee-track">{spans_en}</div></div>'
    elif project["wildcard"] == "metadata_sticker":
        d = WILDCARD_DATA["awjaj"]
        rows_en = "".join(f"<dt>{k}</dt><dd>{v}</dd>" for k, v in d["meta_en"])
        extra_html_before = f'<div class="meta-sticker" id="meta-sticker"><dl>{rows_en}</dl></div>'
        extra_js = """
        ...
        if (sticker) {
          ...
          window.addEventListener('mousemove', (e) => {
            ...
            rafId = requestAnimationFrame(() => {
              sticker.style.transform = `translate(${mx + 24}px, ${my + 24}px) rotate(-1.5deg)`;
              rafId = null;
            });
          });
        ...
        extra_html_after = f'<div class="chips" style="padding:0 40px 56px">{chips_html}</div>'
...
'''
    ...
    return css, extra_html_before, extra_html_after, extra_js


def render_project(project):
    folder = PROJECTS_ROOT / project["folder"]
    sections = collect_sections(folder)

    hero_path_root = folder / "1. Project start" / "1. hero photo.png"
    if hero_path_root.exists():
        hero_rel = PREFIX + hero_path_root.relative_to(PROJECTS_ROOT).as_posix()
    else:
        hero_rel = ""

    wc_css, wc_before, wc_after, wc_js = wildcard_for(project["slug"], project, sections)
    gallery_markup, total_imgs = gallery_html(sections)
...
def main():
    for p in PROJECTS:
        ...
        out_path = VARIANT_ROOT / f"project-{p['slug']}.html"
        out_path.write_text(html, encoding="utf-8")
        print(f"  wrote {out_path.name}")
    print(f"Generated {len(PROJECTS)} variant project pages.")
